...
function logLevelBadgeClasses(level: DentalLog["level"]): string {
  ...
  return "text-secondary font-medium dark:text-emerald-300/90 dark:font-semibold text-[#475569]";
}
        ...
        <div className="flex rounded-[14px] bg-gray-100 dark:bg-slate-800 p-1 gap-1 mt-5">
          {(
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-[11px] text-[13px] font-semibold transition-all active:scale-95 ${
                section === t.id
        ...
        <div className="px-5 space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="shrink-0 px-4 py-2 rounded-[12px] border border-red-200 dark:border-red-900/60 text-[12px] font-semibold text-red-600 dark:text-red-400 active:scale-95 transition-transform"
            >
              Очистить логи
            </button>
          </div>

          <div className="rounded-[16px] border border-[#E2E8F0] dark:border-slate-700 bg-[#F8FAFB] dark:bg-slate-900/80 overflow-hidden">
                ...
                <tbody>
                  {logs.length === 0 ? (
                    ...
                    logs.map((row) => (
                      <tr
                        key={row.id}
                        className="border-b border-[#E2E8F0]/80 dark:border-slate-800 odd:bg-white/60 dark:odd:bg-slate-950/40"
                      >
